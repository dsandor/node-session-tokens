        ...
        methods.validateSession = function(sessionToken, nonce, complete, error){
            ...
            try {
                    ...
                    function(session) {
                        ...
                        {
                            if (parseInt(nonce) < (session.nonce - Options.nonceValueAcceptableDeviation/2)
                                || parseInt(nonce) > (session.nonce + Options.nonceValueAcceptableDeviation/2))
                            {
                        ...
                        if (new Date().getTime() > Date.parse(session.expiration))
                        {
